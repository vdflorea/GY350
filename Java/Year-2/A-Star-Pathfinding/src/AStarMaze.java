...
public class AStarMaze extends JFrame implements Runnable, MouseListener, MouseMotionListener, KeyListener {
	...
	private boolean initialCalcPath = true;
	...
	public void run() {
		...
		while ( 1==1 ) {
			...
			if (isGameRunning) {
				...
				if (initialCalcPath) { // Run once to run reCalcPath() at the beginning
					badguy.reCalcPath(map, player.x, player.y);
					initialCalcPath = false;
				}
    ...
    public void mousePressed(MouseEvent e) {
    	if (!isGameRunning) {
	   	 ...
	   	 prevx=x;
	   	 prevy=y;
		 badguy.reCalcPath(map, player.x, player.y); // Make sure path is recalculated since map has changed
     }
